<? 
  ...
  if (mysql_num_rows($talab)>0) {
    ...
    $export.="</table>";
    $fak=mysql_query("SELECT name from fakultet where id='$fak_id'");
    $fakul=mysql_fetch_assoc($fak);
    $fakultet=$fakul['name'];
    header("Content-Type: application/xls");
    header("Content-Disposition:attachment; filename=$fakultet.xls");
    echo $export;
    print("<script>window.location='sort.php'</script>");
  } else {
  print("<script>window.location='sort.php'</script>");
  }
?>